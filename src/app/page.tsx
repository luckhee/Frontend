...
const getFullImageUrl = (url?: string): string | undefined => {
  ...
  if (!baseURL) {
    console.warn("apiClient baseURL이 설정되지 않았습니다.");
    return url; // baseURL이 없으면 원본 URL 반환
  }
  return `${baseURL}${url}`; // 상대 URL인 경우 baseURL 추가
};
      ...
      try {
        ...
        const processPatents = (patents: Patent[]) => {
          if (!Array.isArray(patents)) {
            console.warn("특허 데이터가 배열이 아닙니다:", patents);
            return [];
          }
        ...
        const processedPopular = processPatents(popularResponse);
        const processedRecent = processPatents(recentResponse);

        setPopularPatents(processedPopular);
        setRecentPatents(processedRecent);

        console.log(
          `특허 데이터 로드 완료 - 인기: ${processedPopular.length}개, 최신: ${processedRecent.length}개`
        );
      } catch (error) {
        ...
        <div className="max-w-7xl mx-auto">
          <div className="bg-white/95 backdrop-blur-sm rounded-2xl p-6 shadow-xl">
            <div className="flex gap-3 mb-4">
              <input
                type="text"
                placeholder="특허명, 키워드, 발명자명으로 검색하세요..."
            ...
            <div className="flex gap-2 flex-wrap">
              <button className="bg-blue-100 text-[#1a365d] px-3 py-1 rounded-full text-xs hover:bg-blue-200 transition-colors">
                #인기특허
              </button>
              <button className="bg-blue-100 text-[#1a365d] px-3 py-1 rounded-full text-xs hover:bg-blue-200 transition-colors">
                #신규등록
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {popularPatents.map((patent) => (
                <Link
                  ...
                  className="border border-gray-200 rounded-xl p-4 hover:shadow-lg transition-all duration-300 cursor-pointer transform hover:-translate-y-1 bg-white/50 flex flex-col"
                >
                  ...
                  <div className="flex flex-col flex-grow">
                    <h3 className="font-bold text-[#1a365d] mb-2 text-sm flex-grow">
                      {patent.title}
                    </h3>
