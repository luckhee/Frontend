...
export default function PatentDetailPage() {
  ...
  const handlePurchaseInquiry = async () => {
    ...
    try {
      ...
      if (response.data.resultCode === "200") {
        ...
        try {
          // 채팅방 목록 새로고침 (새로 생성된 채팅방이 목록에 포함되도록)
          await refreshChatRooms();
          console.log("채팅방 목록 새로고침 완료");

          // 채팅방 목록이 업데이트될 시간을 주고 리다이렉트
          setTimeout(() => {
            console.log(`채팅 페이지로 리다이렉트: /chat?roomId=${chatRoomId}`);
            router.push(`/chat?roomId=${chatRoomId}`);
          }, 500); // 300ms -> 500ms로 증가하여 목록 업데이트 시간 확보
        } catch (refreshError) {
          console.error(
            "채팅방 목록 새로고침 실패, 직접 리다이렉트:",
            refreshError
          );
          // 새로고침 실패해도 리다이렉트 (ChatRoomWithParams에서 직접 조회)
          router.push(`/chat?roomId=${chatRoomId}`);
        }
      } else {
        console.error(
          "채팅방 생성 실패 - resultCode:",
      ...
      if (
        ...
        try {
          ...
          // 백엔드에서 이미 현재 사용자가 참여한 채팅방만 반환하므로 추가 필터링 불필요
          const rooms = Array.isArray(roomsData) ? roomsData : [];

          if (rooms && rooms.length > 0) {
            const existingRoom = rooms.find(
              (room: { postId: number }) => room.postId === post.id
            );
            if (existingRoom) {
              setTimeout(() => {
        ...
        try {
          ...
          if (
            ...
            if (axiosError.response?.status === 405) {
              console.log(
                "DELETE 메서드가 지원되지 않습니다. POST로 토글합니다."
              );
  ...
  const handleBuy = async () => {
    ...
    try {
      ...
      if (
        ...
        err !== null &&
        "response" in err &&
        typeof (err as { response?: { data?: { msg?: string } } }).response
          ?.data?.msg === "string"
      ) {
        alert(
  ...
  const categoryStyle = colorMap[post.category] || {
    bg: "bg-gray-100",
    text: "text-gray-600",
  };
